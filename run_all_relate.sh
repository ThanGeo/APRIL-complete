...
# relate predicates
declare -a relate_predicates=("disjoint"
                              "intersect"
                              "meet"
                              "equal"
                              "inside"
...
do
    ...
    do
        relation=($j)
        echo ${pair[0]} ${pair[1]} ": " ${relation} >> $build_dir_output_file
        ./main -q ${relation} -s ST2 -R ${pair[0]} -S ${pair[1]} -v $build_dir_csv_file >> $build_dir_output_file
        ./main -q ${relation} -s OP2 -R ${pair[0]} -S ${pair[1]} -v $build_dir_csv_file >> $build_dir_output_file
        ./main -q ${relation} -s ST3 -R ${pair[0]} -S ${pair[1]} -v $build_dir_csv_file >> $build_dir_output_file
        ./main -q ${relation} -s OP3 -R ${pair[0]} -S ${pair[1]} -v $build_dir_csv_file >> $build_dir_output_file
    done
done
